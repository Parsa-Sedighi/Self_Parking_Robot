...
# 3. AI & VISION THRESHOLDS
CONF_THRESHOLD = 80.0  
MIN_AREA = 500    
# MAX_AREA raised to 250,000 so the AI doesn't ignore the target when it gets huge on screen!
MAX_AREA = 250000 
...
try:
    with dai.Device(pipeline) as device:
        ...
        while True:
            ...
            # OPTIMIZED PREPROCESSING (Otsu + Erosion)
            ...
            for cnt in contours:
                ...
                if MIN_AREA < area < MAX_AREA:
                    ...
                    # Aspect Ratio Filter (Kills shadows and stretched '1's)
                    ...
                    # Hardware Buffer Alignment (Forces uint8)
                    ...
                    # RETRIEVE PREDICTION FROM MYRIAD X VPU
                    in_nn = q_nn_out.get()
                    ...
                    if digit == TARGET_DIGIT and conf >= CONF_THRESHOLD:
                        ...
                        if error < -deadzone:
                            ...
                            # --- TERMINAL PARKING SEQUENCE ---
                            # If the bounding box is massive, we are right in front of it!
                            if area > 45000: 
                                print("\n🏁 TARGET REACHED! Executing final blind approach...")
                                robot.surge(0.6, 0.5) # Drive forward blindly for half a second
                                robot.stop(0)
                                print("✅ Mission Accomplished. Shutting down system.")
                                sys.exit(0) # Kills the script entirely so it stays parked
                            else:
                                # Target Centered but still far away, keep tracking forward
                                robot.surge(0.6, 0) 
                        
                        break 

            # --- PULSE SCANNING ---
            # If it lost the target, pulse scan to find it without motion blur
